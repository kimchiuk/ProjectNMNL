...
const product = computed(() => {
  return store.savings?.filter((product) => product.fin_prdt_cd === prdtCd)[0];
});

const filterCart = computed(() => {
  return cartStore.savingsCart.filter(
    (v) => v.product === product.value.id && v.user === counterStoer.userInfo.pk
  );
});

const optionList = computed(() => {
  return store.savingsOptions?.filter(
